...
const validateHomePageContent = (content: any): void => {
...
export const getHomePageContent = async (req: Request, res: Response) => {
    try {
        ...
        if (!doc.exists) {
            logger('error', 'Conteúdo da página inicial não encontrado'); // Adicionando log de erro
            return res.status(404).json({ message: 'Conteúdo da página inicial não encontrado' });
        }
        res.json({ id: doc.id, ...doc.data() });
    } catch (error: any) {
        logger('error', `Erro ao obter conteúdo da página inicial: ${error.message}`); // Adicionando log de erro
...
export const updateHomePageContent = async (req: Request, res: Response) => {
    try {
        ...
        logger('info', 'Conteúdo da página inicial atualizado'); // Adicionando log de sucesso
        res.json({ id: updatedDoc.id, ...updatedDoc.data() });
    } catch (error: any) {
        logger('error', `Erro ao atualizar conteúdo da página inicial: ${error.message}`); // Adicionando log de erro
